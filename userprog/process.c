...
{
	struct thread *curr = thread_current();
	printf("%s: exit(%d)\n",thread_current()->name,curr->sys_status);
	/* TODO: Your code goes here.
	 * TODO: Implement process termination message (see
	 * TODO: project2/process_termination.html).
	 * TODO: We recommend you to implement process resource cleanup here. */
	sema_up(&curr->fork_wait_sema);
	process_cleanup();
}
...
static bool load(const char *file_name, struct intr_frame *if_)
{
	...
	argument_stack(parse, idx, &if_->rsp);
	if_->R.rdi = idx;
	if_->R.rsi = if_->rsp + 8;
...
void argument_stack(char **parse, int idx, void **rsp)
{

	char *argv_address[idx];
	int size = 0;

	for (int i = idx - 1; -1 < i; i--)
	{
		*rsp -= (strlen(parse[i]) + 1);
		memcpy(*rsp, parse[i], strlen(parse[i]) + 1);
	...
	**(char **)rsp = 0;
	...
	{
		*rsp = *rsp - 8;
		memcpy(*rsp, &argv_address[i], strlen(&argv_address[i]) + 1);
	}
